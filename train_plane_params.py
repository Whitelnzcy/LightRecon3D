...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Initialize the model from an existing Stage1 checkpoint.",
    )
...
def compute_token_param_losses(res, batch, args):
    ...
    log = {
        ...
        "normal_angle_deg": to_float(angle_deg.mean()),
        "offset_abs_error": to_float(offset_err.mean()),
        "point_to_plane_residual": to_float(loss_consistency),
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        if step == 1 or step % args.log_every == 0 or step == len(loader):
            ...
            print(
                ...
                f"angle={stats.get('normal_angle_deg', 0.0):.2f}deg "
                f"offset={stats['loss_param_offset']:.4f} "
                f"offset_abs={stats.get('offset_abs_error', 0.0):.4f} "
...
def main():
    ...
    if args.init_checkpoint:
        print("This run fine-tunes an existing Stage1 parameter checkpoint.")
    else:
        print("This run trains a fresh Stage1 parameter head.")
