...
class AsymmetricCroCo3DStereo (
    ...
    def forward(self, view1, view2):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            # 为了方便截胡，我们把 float 转换提出来
            dec1_float = [tok.float() for tok in dec1]
            dec2_float = [tok.float() for tok in dec2]
            
            res1 = self._downstream_head(1, dec1_float, shape1)
            res2 = self._downstream_head(2, dec2_float, shape2)

        res2['pts3d_in_other_view'] = res2.pop('pts3d')  # predict view2's pts3d in view1's frame

        # === 【LightRecon3D 截胡代码开始】 ===
        # dec_float[-1] 是最后一层 Decoder 输出，形状为 (B, S, D)
