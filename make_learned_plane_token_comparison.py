...
def main():
    ...
    valid_assignment = assignment[assignment >= 0]
    counts = np.bincount(valid_assignment, minlength=len(plane_normals))
    if args.edit_plane == "largest":
        if counts.sum() == 0:
            raise ValueError("No active plane assignments found; all points are background/no-plane.")
