...
class Structured3DDataset(Dataset):
    def __init__(
        ...
        self.plane_offset_scale = plane_offset_scale
    ...
    def generate_masks(self, json_data, height=720, width=1280):
        ...
        plane_normal = np.zeros((height, width, 3), dtype=np.float32)
        plane_offset = np.zeros((height, width), dtype=np.float32)
        plane_param_valid = np.zeros((height, width), dtype=np.uint8)

        all_junctions = json_data["junctions"]

        for i, plane in enumerate(json_data.get("planes", [])):
            plane_id = int(i + 1)
            normal = np.array(plane.get("normal", [0.0, 0.0, 0.0]), dtype=np.float32)
            normal_norm = float(np.linalg.norm(normal))
            if normal_norm > 1e-8:
                normal = normal / normal_norm
            offset = float(plane.get("offset", 0.0)) / float(self.plane_offset_scale)
            ...
            for mask_indices in plane.get("visible_mask", []):
                ...
                cv2.fillPoly(plane_mask, [poly_points], color=plane_id)
                cv2.polylines(line_mask, [poly_points], isClosed=True, color=255, thickness=2)
                cv2.fillPoly(plane_normal, [poly_points], color=normal.tolist())
                cv2.fillPoly(plane_offset, [poly_points], color=offset)
                cv2.fillPoly(plane_param_valid, [poly_points], color=1)

        return plane_mask, line_mask, plane_normal, plane_offset, plane_param_valid
    ...
    def __getitem__(self, idx):
        ...
        plane_mask, line_mask, plane_normal, plane_offset, plane_param_valid = self.generate_masks(layout_data)
        ...
        plane_normal = cv2.resize(
            plane_normal, (target_w, target_h), interpolation=cv2.INTER_NEAREST
        )
        plane_offset = cv2.resize(
            plane_offset, (target_w, target_h), interpolation=cv2.INTER_NEAREST
        )
        plane_param_valid = cv2.resize(
            plane_param_valid, (target_w, target_h), interpolation=cv2.INTER_NEAREST
        )
        ...
        sample = {
            ...
            "gt_plane_normal": torch.from_numpy(plane_normal).permute(2, 0, 1).float(),  # [3,H,W]
            "gt_plane_offset": torch.from_numpy(plane_offset).unsqueeze(0).float(),      # [1,H,W]
            "gt_plane_param_valid": torch.from_numpy(plane_param_valid).unsqueeze(0).float(),  # [1,H,W]
        }
